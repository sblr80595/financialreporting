"""BS Schedule Finalyzer API routes."""
...
@router.post("/generate-bs-schedule", response_model=BSScheduleGenerationResponse)
async def generate_bs_schedule(
    ...
    convert_to_lakh: bool = Query(
        False,
        description="Convert values to lakhs by dividing by 100,000"
    ),
):
    """
    Generate BS Schedule Finalyzer with detailed line items from Balance Sheet notes.

    This endpoint:
    1. Checks if all required notes are available (Notes 3-8, 10-15, 17-23)
    ...
    4. Generates schedule with:
       ...
       - "BS Schedule" section marker (yellow)
       - Individual note sections with line items
       - Orange headings for main sections
       - Totals at the end of each section
    ...
    6. Value conversion options:
       - convert_to_lakh=True: Divides by 100,000
       - convert_to_lakh=False: Shows actual values
    7. Exports to Excel matching template format
    8. Saves in financial_statements/{company_name}/BS_Schedule/ directory
    ...
    Args:
        ...
        show_currency_prefix: Show currency prefix or just numbers
        currency_prefix: Currency prefix to use
        convert_to_lakh: Convert values to lakhs

    Returns:
        BSScheduleGenerationResponse with file path
    """
    ...
    if not readiness["is_ready"]:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "Cannot generate BS Schedule. Not all required notes are available.",
    ...
    result = BSScheduleFinalyzerService.generate_bs_schedule(
...
@router.get("/download-bs-schedule/{company_name}")
async def download_latest_bs_schedule(company_name: str):
    """
    Download the most recently generated BS Schedule Excel file.
    ...
    if not schedule_dir.exists():
        raise HTTPException(
            status_code=404,
            detail=f"No BS Schedule found for {company_name}",
        )

    # Find the most recent BS Schedule file
    schedule_files = list(schedule_dir.glob("BS_Schedule_*.xlsx"))
    ...
    return FileResponse(
        ...
        filename=f"BS_Schedule_{company_name}.xlsx",
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )


@router.get("/bs-schedule-list/{company_name}")
async def list_bs_schedules(company_name: str):
    """
    List all generated BS Schedules for a company with metadata.

    Args:
        company_name: Name of the company

    Returns:
        List of generated BS Schedules with timestamps and file paths
    """
    from backend.services.path_service import PathService
        ...
        schedules.append(
            {
                ...
                "download_url": f"/api/download-bs-schedule/{company_name}",
            }
        )
...
@router.get("/bs-schedule-readiness/{company_name}")
async def check_bs_schedule_readiness(company_name: str):
    """
    ...
    try:
        ...
        return {
            **readiness,
            "statement_type": "BS_Schedule",
            "message": "BS Schedule readiness check",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error checking BS Schedule readiness: {str(e)}"
        )