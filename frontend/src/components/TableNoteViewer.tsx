...
const TableNoteViewer: React.FC<TableNoteViewerProps> = ({
  noteNumber,
  noteName,
  content,
  companyName,
  ...
  useEffect(() => {
    ...
    const extractedTables = parseMarkdownTables(content);
    setTables(extractedTables);
  }, [content]);

  const handleDownload = () => {
    const filename = formatNoteFileName(noteNumber, noteName, companyName);
    const sheetNames = tables.map((_, index) => 
      noteName ? `${noteName}_${index + 1}` : `Table_${index + 1}`
    );
...
export default TableNoteViewer;
